 ...
 */
public class LoomUnitExtension
    ...
    public void beforeEach(ExtensionContext extensionContext) throws InterruptedException {
        if (requiresRecording(extensionContext.getRequiredTestClass(), extensionContext.getRequiredTestMethod())) {
            ...
            if (getShouldPin(extensionContext.getRequiredTestClass(), extensionContext.getRequiredTestMethod()) != null
                    &&
                    getShouldNotPin(extensionContext.getRequiredTestClass(), extensionContext.getRequiredTestMethod())
                            != null) {
    ...
    public void afterEach(ExtensionContext extensionContext) throws InterruptedException {
        ...
        List<RecordedEvent> pinEvents =
                captured.stream().filter(re -> re.getEventType().getName().equals(CARRIER_PINNED_EVENT_NAME))
        ...
        if (pin != null) {
            if (pinEvents.isEmpty()) {
                throw new AssertionError("The test " + extensionContext.getDisplayName()
                        + " was expected to pin the carrier thread, it didn't");
            }
        ...
        if (notpin != null) {
            if (!pinEvents.isEmpty()) {
                List<RecordedEvent> overTimePinedEvents = pinEvents.stream()
                        .filter(e -> e.getDuration().getNano() > Duration.of(notpin.threshHold(), notpin.unit())
                                .getNano())
                        .toList();
                if (!overTimePinedEvents.isEmpty()) {
                    throw new AssertionError("The test " + extensionContext.getDisplayName()
                            + " was expected to NOT pin the carrier thread"
                            + " over threshhold, but we collected " + overTimePinedEvents.size() + " event(s)\n" + dump(
                            pinEvents));
                }
            }
        }
    ...
    private String dump(List<RecordedEvent> pinEvents) {
        ...
        for (RecordedEvent pinEvent : pinEvents) {
            ...
            builder.append("duration:").append(pinEvent.getDuration().get(ChronoUnit.NANOS)).append(" ns \n");
